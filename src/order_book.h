...
struct alignas(64) PriceLevel {
    ...
    PriceLevel(uint64_t p) : price(p), head(nullptr), tail(nullptr), total_quantity(0) {}
};
...
private:
    ...
    inline void execute_trade(Order* maker, uint32_t quantity);

    std::vector<PriceLevel*> bids;
    std::vector<PriceLevel*> asks;
    std::unordered_map<uint64_t, Order*> order_map;
    MemoryPool<Order> order_pool;
    MemoryPool<PriceLevel, 1000> level_pool;
